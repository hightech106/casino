"use strict";
/**
 * Module providing users shop functionality.
 */
/**
 * User shop interactions controller. Manages user purchases from reward shop.
 */
      ...
      if ((shop.type_gift === 'cash_bonus' || shop.type_gift === 'free_bet') && shop.type_pay === 'points') {
        ...
        if (userPointsCost) {
          ...
          if (shop.type_gift === 'cash_bonus') {
            calculatedPayout = shop.cost * priorityValue;
...
const buyItem = async (req, res) => {
  try {
    ...
    // Only crypto/points payments accepted - fiat payments removed
    if (shop.type_pay === 'fiat') {
      return res.status(403).json({ message: 'Fiat payments not supported. Only crypto and points payments are accepted.' });
    ...
    // Only crypto/points payments accepted - fiat payments removed
    if (shop.type_pay === 'fiat') {
      return res.status(403).json({ message: 'Fiat payments not supported' });
    } else {
