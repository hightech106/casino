...
// api
import { clickConversation } from '../chat.api';
// types
import type { IChatConversation } from '../chat.type';
//
...
export default function ChatNavItem({ selected, collapse, conversation, onCloseMobile }: Props) {
  ...
  const handleClickConversation = useCallback(async () => {
    try {
      ...
      router.push(`/${currentLang.value}${paths.user.support}?id=${conversation.id}`);
    } catch (error) {
      console.error(error);
    }
  }, [conversation.id, mdUp, onCloseMobile, router, currentLang]);
