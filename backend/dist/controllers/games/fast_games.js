"use strict";
/**
 * Module providing fast games functionality.
 */
/**
 * Fast games controller. Handles instant/crash-style game operations.
 */
...
exports.get = exports.getOne = exports.replaceInvalid = exports.list = exports.label = exports.create = exports.updateOne = exports.deleteOne = void 0;
const base_1 = require("../base");
const models_1 = require("../../models");

const get = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    const fastGames = yield models_1.FastGameLists.find().sort({ number: 1, createdAt: -1 });
    ...
    res.json(
      games.map((game, index) => ({
        _id: fastGames[index]._id,
        number: fastGames[index].number,
...
const getOne = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    ...
    const fastGame = yield models_1.FastGameLists.findOne({ _id: base_1.ObjectId(req.params.id) });
    if (!fastGame) {
      return res.status(404).json({ error: "Fast game not found" });
    }
    const game = yield models_1.GameLists.findOne({ game_code: fastGame.game_code });
    if (!game) {
...
const replaceInvalid = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    ...
    const result = yield models_1.FastGameLists.findOneAndUpdate(
...
const list = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    ...
    const fastGames = yield models_1.FastGameLists.find(query)
      .sort({ number: 1, createdAt: -1 })
      .limit(pageSize)
      .skip((page - 1) * pageSize);
    const count = yield models_1.FastGameLists.countDocuments(query);
...
const label = async (req, res) => {
  try {
    ...
    const fastGames = await models_1.FastGameLists.find(query)
      .sort({ number: 1, createdAt: -1 })
      .skip((page - 1) * pageSize)
      .limit(pageSize);
    ...
    let results;
    if (gamecode) {
      results = fastGames.map((fastGame) => {
        const game = gameMap.get(fastGame.game_code);
        return {
...
const create = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    ...
    const existingNumber = yield models_1.FastGameLists.findOne({ number });
    if (existingNumber) {
      return res.status(400).json({ error: "Number already exists" });
    }
    const existingGameCode = yield models_1.FastGameLists.findOne({ game_code });
    if (existingGameCode) {
      return res.status(400).json({ error: "Game code already assigned to another number" });
    }
    const result = yield models_1.FastGameLists.create({ number, game_code });
    res.json(result);
  } catch (error) {
...
const updateOne = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    const result = yield models_1.FastGameLists.findByIdAndUpdate(
...
const deleteOne = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
  try {
    const result = yield models_1.FastGameLists.deleteOne({ _id: (0, base_1.ObjectId)(req.params.id) });
