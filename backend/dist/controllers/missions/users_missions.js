"use strict";
/**
 * Module providing users missions functionality.
 */
/**
 * User missions controller. Tracks user mission progress and completion status.
 */
...
const checkSportMission = async (mission, userId, balance) => {
  ...
  // minStakeInUSD is already in LU
...
const checkWinsMission = async (mission, userId, balance) => {
  ...
  // All amounts are in LU (USD), no conversion needed
  const requiredWin = win;
  ...
  if (min_deposit > 0) {
    ...
    for (const deposit of deposits) {
      // Use fiat_amount (LU) if available, otherwise amount
      totalDeposited += deposit.fiat_amount || deposit.amount || 0;
    }
  ...
  for (const winEntry of wins) {
    // All win amounts are in LU, no conversion needed
    totalWin += winEntry.win_money || 0;
  }

  return { totalWin, requiredWin, depositMet };
};
...
const checkDepositMission = async (mission, userId, balance) => {
  ...
  // All amounts are in LU (USD), no conversion needed
  const requiredDeposit = min_deposit;
  ...
  return { totalDeposited, requiredDeposit };
};
