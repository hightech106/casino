"use strict";
/**
 * Index file exporting reports modules and routes.
 */
/**
 * Reports controller. Generates system reports and analytics.
 */
...
const getSportsBetData = (qdate, req) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    const balances = yield models_1.Balances.aggregate([
        ...
        },
        {
            $project: {
                ...
                usd: '$balance', // price removed - always 1 (LU = USD)
            },
        },
        {
            $sort: {
    ...
    const payments = yield models_1.Payments.aggregate([
        ...
        },
        {
            $project: {
                amount: 1,
                usd: '$amount', // price removed - always 1 (LU = USD)
                count: 1,
                _id: 0,
                currency: '$_id.currency.symbol',
                order: '$_id.currency.order',
...
const getProfits = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        return res.json({
            // price removed - always 1 (LU = USD)
            profit: (0, base_1.toNumber)(profit * 0.1, 2),
            icon: mbt.icon,
        });
    }
    catch (error) {
    ...
    const result1 = yield models_1.SportsBets.aggregate([
        ...
        },
        {
            $group: {
                ...
                profit: {
                    $sum: {
                        $subtract: ['$stake', '$profit'], // price removed - always 1 (LU = USD)
                    },
                },
            },
        },
    ...
    const result2 = yield models_1.SportsBets.aggregate([
        ...
        },
        {
            $group: {
                ...
                total: { $sum: 1 },
                profit: {
                    $sum: '$profit', // price removed - always 1 (LU = USD)
                },
            },
        },
        {
    ...
    const result3 = yield models_1.GameHistories.aggregate([
        ...
        },
        {
            $group: {
                ...
                profit: {
                    $sum: {
                        $subtract: ['$bet_money', '$win_money'], // price removed - always 1 (LU = USD)
                    },
                },
            },
        },
