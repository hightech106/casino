/**
 * Admin dashboard API controller providing analytics and reporting endpoints.
 * Handles financial activity reports, user statistics, bonus analytics, and country-based user data.
 * Used by admin panel to display business metrics and user insights.
 */
"use strict";
...
const get_bonus_details = (startDate, endDate, type, provider_code, game_code) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        for (const history of bonusHistories) {
            ...
            // All amounts are already in LU - no conversion needed
        ...
        if (!type || type === 'casino') {
            ...
            for (const game of gameHistories) {
                ...
                // All amounts are already in LU - no conversion needed
                let amountUsd = game.txn_type === 'BET' ? game.bet_money || 0 : game.win_money || 0;
...
const get_balance_reports = (startDate, endDate) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [currency, data] of Object.entries(balanceByCurrency)) {
        ...
        try {
            if (currency === 'USDT') {
                ...
                // All amounts are in LU - EUR conversion removed
                balanceReports.find((report) => report.name === 'USDT').eur = data.totalBalance;
                totalEur += data.totalBalance;
            } else if (data.isFiat) {
                // All amounts are in LU - no conversion needed
                usdRate = 1;
                eurRate = 1;
                balanceReports.find((report) => report.name === 'Fiat').value += data.totalBalance;
                balanceReports.find((report) => report.name === 'Fiat').eur += data.totalBalance;
                balanceReports.find((report) => report.name === 'Fiat').usd += data.totalBalance;
                totalUsd += data.totalBalance;
                totalEur += data.totalBalance;
            } else {
                const cryptoCurrency = utils_1.AVAILABLE_COINS.find((coin) => coin.symbol === currency);
                if (cryptoCurrency) {
                    // Only LU (USD) supported - no EUR conversion needed
                    const fiatRate = yield fiatConverter_1.getFiatToCryptoRate(cryptoCurrency, 'USD', 1);
                    if (!fiatRate) {
                        console.error(`DEBUG: Failed to fetch crypto rate for ${currency}`);
                        continue;
                    }
                    usdRate = 1 / fiatRate.crypto_amount;
                    eurRate = usdRate; // EUR same as USD since we only use LU
    ...
    for (const crypto of cryptoCurrencies) {
        const cryptoCurrency = utils_1.AVAILABLE_COINS.find((coin) => coin.symbol === crypto);
        if (!cryptoCurrency) {
            console.error(`DEBUG: Crypto currency not found for ${crypto}`);
            continue;
        }
        try {
            const usdRateResult = yield fiatConverter_1.getFiatToCryptoRate(cryptoCurrency, 'USD', totalUsd);
            if (!usdRateResult) {
    ...
    // All amounts are in LU - EUR conversion removed
    balanceReports.find((report) => report.name === 'USDT').eur = totalUsd;
...
const get_total_deposits = (startDate, endDate) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [currencyId, data] of Object.entries(depositByCurrency)) {
        try {
            // All amounts are already in LU (USD), no conversion needed
            if (data.isFiat) {
                // LU is fiat, add directly
                depositReports.find((report) => report.name === 'Fiat').value += data.totalAmount;
                depositReports.find((report) => report.name === 'Fiat').eur += data.totalAmount; // EUR = USD for reporting
                depositReports.find((report) => report.name === 'Fiat').usd += data.totalAmount;
                totalFiatUsd += data.totalAmount;
                totalFiatEur += data.totalAmount;
            } else {
                // Crypto deposits: totalFiatAmount is already in LU (USD)
                ...
                if (cryptoDetail) {
                    cryptoDetail.value = data.totalAmount; // Crypto amount
                    cryptoDetail.usd = data.totalFiatAmount; // Already in LU (USD)
                    totalCryptoUsd += data.totalFiatAmount;
                }
            }
        } catch (apiError) {
            console.error('DEBUG: Error processing deposit for', data.symbol, apiError.message);
            continue;
        }
    }
...
const get_total_withdrawals = (startDate, endDate) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [currencyId, data] of Object.entries(withdrawalByCurrency)) {
        try {
            ...
            if (data.isFiat) {
                // LU is fiat, add directly
                withdrawalReports.find((report) => report.name === 'Fiat').value += data.totalAmount;
                withdrawalReports.find((report) => report.name === 'Fiat').eur += data.totalAmount; // EUR = USD for reporting
                withdrawalReports.find((report) => report.name === 'Fiat').usd += data.totalAmount;
                totalFiatUsd += data.totalAmount;
                totalFiatEur += data.totalAmount;
            } else {
                // Crypto withdrawals: totalFiatAmount is already in LU (USD)
            ...
            console.error('DEBUG: Error processing withdrawal for', data.symbol, apiError.message);
            continue;
        }
    }
...
const get_total_wins_gaming = (startDate, endDate, type, provider_code, game_code) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    for (const [currencyId, data] of Object.entries(winByCurrency)) {
        try {
            ...
            if (data.isFiat) {
                // All amounts are in LU - no conversion needed
                fiatUsd += data.totalAmount;
                fiatEur += data.totalAmount;
