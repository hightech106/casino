/**
 * Cron job scheduler that runs periodic tasks for the application.
 * Handles scheduled tasks like deposit confirmation, bonus distribution, activity calculations,
 * and automated communications. Runs on configurable intervals (one-minute and daily schedules).
 */
"use strict";
        ...
        if (marketings.length) {
            for (const i in marketings) {
                ...
                if (event === 'email' && toPlayers.length) {
                    ...
                    console.log('cron.js:233: Sending mailOptions:', mailOptions);
