/**
 * Main Socket.IO server configuration and connection handler.
 * Manages real-time WebSocket connections, session tracking, popup player management,
 * and site-wide updates for live features like mini-app players and popups.
 */
"use strict";
...
exports.default = (io) => {
    io.on('connection', (socket) => __awaiter(void 0, void 0, void 0, function* () {
        console.log("[MAIN SOCKET] 🟢 New connection:", socket.id, "from:", socket.handshake.address, "origin:", socket.handshake.headers.origin);
        let user = null;
        socket.on('auth', (token) => __awaiter(void 0, void 0, void 0, function* () {
            console.log("[MAIN SOCKET] 🔑 Auth request from socket:", socket.id, "token:", token ? token.substring(0, 10) + '...' : 'missing');
            try {
                const decoded = yield models_1.Sessions.findOneAndUpdate({ accessToken: token }, { socketId: socket.id }, { new: true, upsert: true });
                if (decoded) {
                    console.log("[MAIN SOCKET] ✅ Auth successful for userId:", decoded.userId, "socketId:", socket.id);
                    ...
                    console.log("[MAIN SOCKET] ❌ Auth failed: session not found for token");
                    io.to(socket.id).emit('logout');
                }
            }
            catch (err) {
                console.error("[MAIN SOCKET] ❌ Auth error:", err);
                io.to(socket.id).emit('logout');
            }
        }));
        socket.on('disconnect', () => __awaiter(void 0, void 0, void 0, function* () {
            ...
            if (session && session.userId) {
                ...
                // Don't immediately clear socketId - wait a bit in case of quick reconnection
                // This helps with balance updates during brief disconnects
                setTimeout(() => __awaiter(void 0, void 0, void 0, function* () {
                    const currentSession = yield models_1.Sessions.findOne({ userId: session.userId, socketId: socket.id });
                    // Only clear if this socket is still disconnected (no reconnection happened)
                    if (currentSession && currentSession.socketId === socket.id) {
                        yield models_1.Sessions.updateOne({ socketId: socket.id }, { socketId: '' });
                        console.log("[MAIN SOCKET] 🧹 Cleared socketId for disconnected socket:", socket.id);
                    }
                }), 3000); // Wait 3 seconds before clearing
            } else {
                // No session found, clear immediately
                yield models_1.Sessions.updateOne({ socketId: socket.id }, { socketId: '' });
            }
        }));
    }));
