/**
 * Sports betting utility functions for market names, odds conversion, betslip management, and live event locking.
 * Handles complex market naming logic, handicap conversion, and betslip data transformation for different sports.
 * Note: Contains sport-specific logic for soccer (1), basketball (18), tennis (17), and baseball (16) with complex locking rules.
 */
import type {
  BetslipProps,
  ISportsEvent,
...
export const getName = (activeSports: SportsListProps | undefined) => {
  /* eslint-disable */
  if (
    activeSports?.SportId === 1 ||
    activeSports?.SportId === 18 ||
    activeSports?.SportId === 17 ||
    activeSports?.SportId === 16
  ) {
...
export const getMarkets = (event: ISportsEvent, activeSports: SportsListProps | undefined) => {
  ...
  if (keyTwo && markets[keyTwo]) {
    if (getMarket(markets[keyTwo])) {
      marketTwo = {
        ...getMarket(markets[keyTwo]),
        marketName: getMarketName(keyTwo),
        marketId: keyTwo,
      };
    }
  }
  if (keyThree && markets[keyThree]) {
    if (getMarket(markets[keyThree])) {
      marketThree = {
        ...getMarket(markets[keyThree]),
        marketName: getMarketName(keyThree),
        marketId: keyThree,
      };
    }
  }
...
export const getIsLock = ({ item, sports, isLive, event }: LockProps) => {
  ...
  if (isLive) {
    ...
    if (
      ...
      if (
        (marketId === '1_1' || marketId === '1_2' || marketId === '1_3' || marketId === '1_4') &&
        Number(tm) > 80
      ) {
        return true;
      } else if (
        (marketId === '1_5' || marketId === '1_6' || marketId === '1_7' || marketId === '1_8') &&
        Number(tm) > 38
      ) {
        return true;
      }
      ...
      if (
        ...
        (marketId === '18_4' || marketId === '18_5' || marketId === '18_6') &&
        (Number(q) > 2 || Number(tm) <= 5)
      ) {
        return true;
      } else if (
        (marketId === '18_7' || marketId === '18_8' || marketId === '18_9') &&
        (Number(tm) < 5 || item.q !== q)
      ) {
        return true;
      }
  ...
  if (matchs.length) {
    for (const i in matchs) {
      if (
        matchs[i]?.LeagueId &&
        params[i]?.LeagueId &&
        matchs[i]?.LeagueId === params[i]?.LeagueId
      ) {
        for (const j in matchs[i].events) {
          if (matchs[i].events[j]) {
            ...
            if (
              preEvent?.id &&
              params[i].events[j]?.id &&
              preEvent?.id === params[i].events[j]?.id
            ) {
...
export const formatData = (event: any): MarketProps[] => {
  ...
  for (const key in odds) {
    if (odds[key]) {
      ...
      const data =
        !odds[key].handicap || (odds[key].handicap && odds[key].handicap.indexOf(',') === -1);
      if (data) {
        odd.data = odds[key];
...
export const getoddTypeData = (name: string) => {
  if (name === "1") {
    return "Home";
  }
  if (name === "2") {
...
export const convertBetslipData = ({
  event,
  odd,
  oddType,
  sports,
  isBet365 = false
}: SelectOddProps): BetslipProps => {
  let oddName = '';
  const oddId = odd.id;
  const marketId = odd.marketId;
  const marketName = odd.marketName;
  if (isBet365) {
    oddName = `${getoddTypeData(odd.name) || odd.handicap}${odd.header ? ` (${getoddTypeData(odd.header)})` : ""}`;
  };

  if (
    marketId.indexOf('_1') !== -1 ||
    marketId === '1_8' ||
    marketId === '18_4' ||
    marketId === '18_7' ||
    marketId === '3_4'
  ...
  return {
    ...
    isBet365,
    oddType,
    odds: Number(!isBet365 ? odd[`${oddType}_od`] : odd.odds),
    oddData: odd,
    stake: 0,
    sports,
  };
...
export const convertSoccerBetslipData = ({
  event,
  odd,
  sports,
}: any): BetslipProps => {
  // Generate oddName based on oddType
  let oddName = '';
  const oddType = odd.oddType || odd.value || '';
  
  if (oddType === 'home' || oddType === 'Home' || oddType === '1') {
    oddName = event.home.name;
  } else if (oddType === 'away' || oddType === 'Away' || oddType === '2') {
    oddName = event.away.name;
  } else if (oddType === 'draw' || oddType === 'Draw' || oddType === 'X') {
    oddName = 'Draw';
  } else if (oddType === 'over' || oddType === 'Over') {
    oddName = `Over ${odd.handicap || '2.5'}`;
  } else if (oddType === 'under' || oddType === 'Under') {
    oddName = `Under ${odd.handicap || '2.5'}`;
  } else if (oddType === 'btts') {
    oddName = 'Both Teams To Score - Yes';
  } else if (oddType === 'btts_no') {
    oddName = 'Both Teams To Score - No';
  } else {
    oddName = odd.value || oddType || 'Unknown';
  }
  ...
  return {
    ...
    AwayTeam: event.away.name,
    HomeTeam: event.home.name,
    marketId: odd.marketId?.toString() || odd.id?.toString() || '0',
    marketName: odd.marketName || 'Unknown Market',
    oddId: odd.id?.toString() || odd.marketId?.toString() || '0',
    oddName,
    oddType,
    odds: Number(odd.odds || odd.odd || 0),
    oddData: odd,
...
export const addRemoveBetslip = (betslipData: BetslipProps[], betslip: BetslipProps) => {
  console.log(betslip, "===>betslip", betslipData);
...
export const checkActive = (betslipData: BetslipProps[], id: string, oddType: IOddTypes) =>
...
export const abbreviate = (value: any, decimal = 2) => {
  ...
  return (
    Number((value / Math.pow(1000, index)).toFixed(1).replace(/\.0$/, '')).toFixed(decimal) +
    suffix[index - 1]
  );
  /* eslint-enable */
};
