/**
 * Journey flow processing microservice that connects to main Socket.IO server.
 * Handles user journey tracking, triggers custom journey events, and processes journey conditions.
 * Runs as a separate process to isolate journey logic from main API server.
 */
// journey-flow.js
'use strict';
...
const API_URL = process.env.API_URL || 'https://luckverse777.com';
...
try {
    mongoose_1.default.connect(process.env.DATABASE).then(async () => {
        await redis_1.redisClient.ping().then(() => console.log('[Journey] In-memory store ready'));
