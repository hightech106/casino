...
import HexagonSvg from 'src/assets/images/games/hexagon.svg';
import hexblue from 'src/assets/images/games/hex-blue.webp';
import hexwhite from 'src/assets/images/games/hex-white.webp';
import hexdark from 'src/assets/images/games/hex-dark.webp';
import hexorange from 'src/assets/images/games/hex-orange.webp';
import hexgreen from 'src/assets/images/games/hex-green.webp';
import hexdiamond from 'src/assets/images/games/hex-diamond.webp';

import xb0 from 'src/assets/audio/0x.mp3';
import xb2 from 'src/assets/audio/2x.mp3';
import xb5 from 'src/assets/audio/5x.mp3';
import xb10 from 'src/assets/audio/10x.mp3';
import xb100 from 'src/assets/audio/100x.mp3';
import xb1000 from 'src/assets/audio/1000x.mp3';
...
const Slider: React.FC<SliderProps> = ({ multiplier, elapsedTime, numbers = [] }) => {
  ...
  useEffect(() => {
    ...
    if (!sliderRef.current || !targetCardIndex) return () => {};
    ...
    const targetPosition = -(
      targetCardIndex * (cardWidth + cardMarginRight) -
      containWidth / 2 +
      cardOffset
    );
    ...
    <div className="relative overflow-hidden w-full h-full">
      <div ref={sliderRef} className="flex transition-transform duration-0 items-center h-full">
        {displayNumbers.map((number, index) => {
          ...
          const isCrashedpoint = number === multiplier;
          return (
            <div
              key={index}
              className={`${
                isMobile ? 'min-w-[80px] h-40' : 'min-w-[100px] h-44'
              } rounded-md  mr-2 flex-col overflow-clip`}
            >
              <div
              ...
              >
                <div className="relative flex justify-center items-center p-3">
                  ...
                  <div className="absolute top-1/2 left-1/2 text-[1rem] text-white transform -translate-x-1/2 -translate-y-1/2">
      ...
      <div className="absolute left-1/2 bottom-20 rounded-full h-1/4 border-l-2 border-white transform -translate-x-1/2 pointer-events-none" />
...
export const findTile = (number: number): any => {
  ...
  for (let i = 0; i < sortedColors.length; i++) {
    if (
      !sortedColors[i + 1] ||
      (sortedColors[i].point <= number && sortedColors[i + 1].point > number)
    ) {
