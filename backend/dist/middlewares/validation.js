 ...
 * Provides validation rules for common data types (ObjectId, email, etc.) and error handling.
 * Used across all API routes to ensure request data integrity before processing.
 */
"use strict";
    ...
    Users: {
        Auth: {
            ...
            Signup: Joi.object({
                ...
                currency: Joi.string().min(24).max(24).optional(), // Optional - backend automatically uses LU currency
            ...
            SendWithdrawalVerification: Joi.object({
                email: Joi.string().email().required(),
            }),
    ...
    Payments: {
        ...
        Payment: {
            ...
            SolanaDeposit: Joi.object({
                ...
                from: Joi.string().optional(), // Optional, can be derived from transaction
            }),
            TronDeposit: Joi.object({
                userId: Joi.string().min(24).max(24).required(),
                currencyId: Joi.string().min(24).max(24).required(),
                txn_id: Joi.string().min(20).max(100).required(), // TRON txid is hex, can be 64 chars or with 0x prefix
                from: Joi.string().optional(), // Optional, can be derived from transaction
            }),
            TronSweep: Joi.object({
                userId: Joi.string().min(24).max(24).optional(),
                index: Joi.number().integer().min(0).optional(),
                symbol: Joi.string().valid('TRX', 'USDT', 'USDC').required(),
                amountUi: Joi.number().min(0).optional(),
            }).or('userId', 'index'),
            SolanaSweep: Joi.object({
                userId: Joi.string().min(24).max(24).optional(),
                index: Joi.number().integer().min(0).optional(),
                mint: Joi.string().allow(null, '').optional(), // null or empty = SOL native
                amountUi: Joi.number().min(0).optional(), // If omitted, sweeps max available
                toAddress: Joi.string().min(32).max(44).optional(), // Optional custom destination address
            }).or('userId', 'index'), // At least one of userId or index must be provided
            ...
            calcUsetToCrypto: Joi.object({
                id: Joi.number().required(), // Currency id (unique identifier)
                symbol: Joi.string().optional(), // Optional for backward compatibility
                amount: Joi.number().min(0).required(),
                fiatSymbol: Joi.string().optional(),
            }),
            ConfirmSmartContract: Joi.object({
                id: Joi.number().required(), // Currency id (unique identifier)
                symbol: Joi.string().optional(), // Optional for backward compatibility
                amount: Joi.number().min(0).required(), // Crypto amount (e.g., 0.1 BNB, 100 USDT)
                fiatAmount: Joi.number().min(0).required(), // LU amount (USD-equivalent, e.g., 10 LU)
            ...
            Withdrawal: Joi.object({
                ...
                currency: Joi.alternatives().try(Joi.number(), Joi.string()).required(), // Currency id (number) - preferred, or string for backward compatibility
                address: Joi.string().optional(),
