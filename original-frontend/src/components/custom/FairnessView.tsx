...
import {
    buildPrivateHash, generateCrashPoint,
    combineSeeds, generateRouletteOutcome,
    WHEEL_NUMBERS,
    getHiloMGameCard
} from "src/utils/custom";
...
import {
    CopyIcon, Flower1Icon, Flower2Icon,
    Flower3Icon, Flower4Icon, Flower5Icon, PlantSeedIcon
} from "../svgs";
...
const FairnessView = (props: Props) => {
    const { privateHash, publicSeed, privateSeed, gameId, children, prev } = props;
                ...
                <div className="p-3 bg-[#050514]">
                    {
                        loading ? <div className="flex items-center justify-center h-full">
                            <div className="flex space-x-2">
                                ...
                                <div className="w-3 h-3 bg-yellow-400 rounded-full animate-bounce delay-400" />
                            </div>
                        </div> : <>
                                ...
                                {activeTab === 0 ? <>
                                    ...
                                    <CustomInput
                                        ...
                                        icon={
                                            <button
                                                type="button"
                                                onClick={() => copyToClipboard(_privateSeed === "" ? publicSeed : "")}
                                    ...
                                    <CustomInput
                                        ...
                                        icon={
                                            <button
                                                type="button"
                                                onClick={() => copyToClipboard(_privateSeed === "" ? privateHash : "")}
                                                className="px-1 py-2 w-full ">
                                                <CopyIcon />
                                            </button>
                                        } />
                                    ...
                                    <CustomInput
                                        ...
                                        icon={
                                            <button
                                                type="button"
                                                onClick={() => copyToClipboard(prevPublicSeed)} className="px-1 py-2 w-full ">
                                                <CopyIcon />
                                            </button>
                                        } />
                                    ...
                                    <CustomInput
                                        ...
                                        icon={
                                            <button
                                                type="button"
                                                onClick={() => copyToClipboard(prevPrivateSeed)} className="px-1 py-2 w-full ">
                                                <CopyIcon />
                                            </button>
                                        } />
                                </> : <>
                                    ...
                                    <CustomInput type="string" disabled value={_privateHash} />
...
const RenderContext = (props: RenderContextProps) => {
    const { gameId, privateSeed, publicSeed } = props;
    return {
        ...
        "flower-poker": <FlowerPoker privateSeed={privateSeed} publicSeed={publicSeed} />
    }[gameId] || <></>
}
...
const CrashView = ({ privateSeed, publicSeed }: CrashViewProps) => {
    ...
    return (
        <div className="p-5 border-dashed mt-3 border-[1px] border-[#3dff23b4] rounded-md flex justify-center items-center font-bold text-[20px] text-white">
            {crashPoint !== 0 &&
                (
                    crashPoint < 1.2 && (<div> {parseCommasToThousands(crashPoint)}x</div>) ||
                    (crashPoint >= 1.2 && crashPoint < 2) && (<div >  {parseCommasToThousands(crashPoint)}x</div>) ||
                    (crashPoint >= 2 && crashPoint < 100) && (<div > {parseCommasToThousands(crashPoint)}x</div>) ||
...
const RoulleteView = ({ privateSeed, publicSeed }: CrashViewProps) => {
    const [outcome, setOutcome] = useState(0);
    const visible = privateSeed !== "" && publicSeed !== "";

    const getWheelIndex = (outcomeNumber: number) => {
        console.log(outcomeNumber)
        return WHEEL_NUMBERS[WHEEL_NUMBERS.findIndex((n) => n.Number === outcomeNumber)] || WHEEL_NUMBERS[0];
    }
    ...
    return (
        <div className="p-5 border-dashed mt-3 border-[1px] border-[#3dff23b4] rounded-md flex justify-center items-center font-bold text-[20px]">
            {visible ? <div className={`w-[40%] aspect-[1/1] rounded-full flex justify-center items-center `}
                style={{
                    background:
                        getWheelIndex(outcome).Color === "Green" && "radial-gradient(#0bc912, transparent)" ||
                        getWheelIndex(outcome).Color === "Black" && "radial-gradient(#1f1d1d, transparent)" ||
                        "radial-gradient(#c90b0b, transparent)"
                }}
            >
...
const suits = {
    ...
    "Joker": {
        color: "#e9113c",
        icon: <svg viewBox="0 0 20 19"><g><path d="M16.2761 3.65844C17.2913 3.65844 18.1172 2.83785 18.1172 1.82922C18.1172 0.820593 17.2913 0 16.2761 0C15.2609 0 14.4349 0.820632 14.4349 1.82926C14.4349 2.83789 15.2608 3.65844 16.2761 3.65844Z" /><path d="M1.84113 14.3415C0.825937 14.3415 0 15.1621 0 16.1707C0 17.1794 0.825937 18 1.84113 18C2.85633 18 3.68227 17.1793 3.68227 16.1707C3.68227 15.1621 2.85637 14.3415 1.84113 14.3415Z" /><path d="M18.1589 14.3415C17.1437 14.3415 16.3177 15.1621 16.3177 16.1707C16.3177 17.1794 17.1437 18 18.1589 18C19.1741 18 20 17.1794 20 16.1707C20 15.1621 19.1741 14.3415 18.1589 14.3415Z" /><path d="M10 8.5189C10.5623 7.44127 11.4921 6.58256 12.6231 6.10625L11.8496 5.33782C11.9286 4.2662 12.548 3.3414 13.4374 2.83183C13.3248 2.51817 13.263 2.1808 13.263 1.82922C13.263 1.63005 13.2831 1.43546 13.3207 1.24707H11.569C8.99242 1.24707 6.90367 3.32231 6.90367 5.88224V5.93323C8.24773 6.35238 9.36109 7.29437 10 8.5189Z" /><path d="M5.02082 15.5472C5.02082 15.8687 5.28316 16.1293 5.60676 16.1293H14.3932C14.7168 16.1293 14.9792 15.8687 14.9792 15.5472V13.6351H5.02082V15.5472Z" /><path d="M14.9792 11.8183C15.9066 11.9906 16.6934 12.5645 17.1496 13.3505C17.4653 13.2386 17.8049 13.1772 18.1589 13.1772C18.3593 13.1772 18.5552 13.1972 18.7448 13.2345V10.912C18.7448 8.67355 16.9184 6.85892 14.6654 6.85892C12.4123 6.85892 10.5859 8.67351 10.5859 10.912V12.4709H14.9791V11.8183H14.9792Z" /><path d="M9.41406 10.912C9.41406 8.67359 7.58762 6.85896 5.33461 6.85896C3.0816 6.85896 1.2552 8.67355 1.2552 10.912V13.2345C1.44484 13.1972 1.64066 13.1772 1.84113 13.1772C2.19504 13.1772 2.53469 13.2387 2.85043 13.3506C3.30668 12.5645 4.09336 11.9907 5.02082 11.8183V12.4709H9.41406V10.912Z" /></g></svg>
    }
}
...
enum Flower { RED, BLUE, PURPLE, ORANGE, RAINBOW };
const flowers: Flower[] = [Flower.RED, Flower.BLUE, Flower.PURPLE, Flower.ORANGE, Flower.RAINBOW];
const Flowericons = [<Flower1Icon />, <Flower2Icon />, <Flower3Icon />, <Flower4Icon />, < Flower5Icon />]

const FlowerPoker = ({ privateSeed, publicSeed }: { privateSeed: string, publicSeed: string }) => {

    const [hostFlowers, setHostFlowers] = useState<any[]>([]);
    const [PlayerFlowers, setPlayerFlowers] = useState<any[]>([]);
    ...
    useEffect(() => {
        if (privateSeed !== "" && publicSeed !== "") {
            const hash = combineSeeds(publicSeed, privateSeed);

            const playerFlowers = generateFlowerCombination(hash, 0);
            const hostFlowers = generateFlowerCombination(hash, 10);
            setPlayerFlowers([...playerFlowers.map((f, index) => {
                return { flower: f, checked: true }
            })])
            setHostFlowers([...hostFlowers.map((f, index) => {
                return { flower: f, checked: true }
            })])
        }
    }, [privateSeed, publicSeed])
    ...
    return (
        <div className="w-full flex flex-col min-h-[60px]">
            <div className="flex justify-center">
                {hostFlowers.map((f, index) => {
                    return <button key={f.checked ? `${index}_f` : `${index}_s`} className="w-[23%] flex justify-center items-center aspect-square md:text-lg text-sm text-white rounded-sm relative bg-[#34545f94] border-[1px] border-[#a39e8c]  md:p-2 p-1" disabled={true}>
                        <div className="w-[80%] h-[80%]">
                            <div className={f.checked ? "animate-bloomAni scale-0" : "scale-0"}>
                                {Flowericons[f.flower]}
                            </div>
                        </div>
                    </button>
                })}
