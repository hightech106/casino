...
import {
    setGameAnimation,
    setGameHotKeyEnable, setMaxBetAllow,
    setSoundVolume, visibleGameInfo, visibleHotKeyPanel
} from "src/store/reducers/setting";
import { AnimationIcon, AudioIcon, GameInfoIcon, KeyBoardIcon, MaxBetIcon, SettingIcon } from "../svgs";

interface Props {
    maxBetDisable?: boolean;
    fairness?: any
};

const SettingBar = ({ maxBetDisable, fairness }: Props) => {
    ...
    return <div className="w-full flex justify-between bg-[#213743] rounded-sm p-2  mt-1">
        <div className="flex space-x-2  border-r-2 border-[#b6b6b6b7] px-6 items-center">
            <Tooltip
                ...
                <button
                    type="button"
                    className="text-white font-bold rounded flex items-center fill-white w-4 h-4 hover:fill-[#d8b228]"
                >
                    <SettingIcon />
                </button>
            </Tooltip>
...
interface TooltipProps {
    children: any; tooltipContent: any,
    isOpen: boolean, setIsOpen: Function
}

const Tooltip = ({ children, tooltipContent, isOpen, setIsOpen }: TooltipProps) => {
    ...
    return (
        <div className="relative inline-block w-full">
            {isOpen && (
                <div ref={containt}
                    className="absolute z-10 bg-[#fff] text-white  rounded-md text-sm bottom-[120%] left-1/2 transform tooltip -translate-x-[30%]"
                >
                    {tooltipContent}
                </div>
            )}
...
const ToolTipItem = ({ children, onClick }: { children: any, onClick?: Function }) => (
    <div onClick={() => { onClick && onClick() }}
        className="flex select-none w-full items-center mb-1 px-2 py-1 space-x-1 cursor-pointer hover:bg-[#5353539d]">
        {children}
    </div>
)