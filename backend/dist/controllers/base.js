/**
 * Base controller providing shared utility functions used across all controllers.
 * Includes authentication helpers, balance checks, email sending, rate limiting, encryption,
 * bonus validation, bet processing, and error handling utilities.
 */
"use strict";
...
const NumberFix = (number, decimal = 2) => {
    ...
    // For higher precision (6-18 decimals), use toFixed to preserve precision
    if (decimal > 5 && decimal <= 18)
        return Number(Number(number).toFixed(decimal));
    // Default fallback
    return Number(number.toString().match(/^\d+(?:\.\d{0,2})?/));
};
...
const checkCasinoBonus = ({ userId, amount, isBonusPlay, game_code }) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    // Find the currency configuration in bonus.currencies array (always LU)
    const currencyConfig = bonus.currencies && bonus.currencies.find(c =>
        c.currency === 'LU' || String(c.currency) === String(luCurrency._id)
    );
...
const getUserBalance = (userId) => __awaiter(void 0, void 0, void 0, function* () {
    // Get LU currency (cache this if needed for performance)
    const luCurrency = yield models_1.Currencies.findOne({ symbol: 'LU' });
    if (!luCurrency) {
        throw new Error('LU currency not configured');
    }
    
    // Find or create LU balance for user
    ...
    // If no balance exists, create one (shouldn't happen, but safety check)
    if (!balance) {
    ...
    // Populate currency for compatibility with existing code
    balance = yield models_1.Balances.findById(balance._id).populate('currency');
...
const getUrlMetadata = (url, lastModified) => {
    ...
    // Main pages
    if (url.endsWith('/luckverse777.com/') ||
        url.endsWith('/en') ||
        url.endsWith('/casino') ||
        url.endsWith('/sports') ||
...
const getSiteMap = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const CASINO_ROOT = 'https://luckverse777.com/en/casino';
        const PROMOTION_URL = 'https://luckverse777.com/en/casino/promotion';
        const SPORTS_ROOT = 'https://luckverse777.com/en/sports';
        ...
        URLS.push({ url: 'https://luckverse777.com/en', lastModified: new Date() });
