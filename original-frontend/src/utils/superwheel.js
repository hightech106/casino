...
SuperWheel = (function () {
  ...
  function SuperWheel(element, settings) {
    ...
    if (typeof self.o.slice.selected !== 'object')
      self.o.slice.selected = self.defaults.slice.selected;
    ...
    if (
      (typeof self.o.center.html.template === 'undefined' || self.o.center.html.template) &&
      typeof self.o.center.html.tmpl !== 'undefined'
    )
      self.o.center.html.template = self.o.center.html.tmpl;

    if (
      (typeof self.o.center.image.url === 'undefined' || self.o.center.image.url) &&
      typeof self.o.center.image.src !== 'undefined'
    )
    ...
    self.instanceUid = $(element).data('superWheelData')
      ? $(element).data('superWheelData').instanceUid
      : instanceUid++;
...
SuperWheel.prototype.init = function () {
  ...
  mask
    ...
    .appendTo(Layersvg);
  if (
    self.o.text.orientation === 'v' ||
    self.o.text.orientation === 'vertical' ||
    ((self.o.text.type === 'icon' || self.o.text.type === 'image') &&
      (self.o.text.orientation === 'h' || self.o.text.orientation === 'horizontal'))
  ) {
    var Layertext = $('<div/>'),
      textHtml = $('<div/>');
    ...
    textHtml.css({
      '-webkit-transform':
        'rotate(' + (-(360 / self.totalSlices()) / 2 + self.getDegree()) + 'deg)',
  ...
  Layercenter.appendTo(
    self.o.center.rotate === true || self.o.center.rotate === 1 || self.o.center.rotate === 'true'
      ? spinner
      : inner
  );
  if (typeof self.o.center.image.url === 'string' && $.trim(self.o.center.image.url) !== '') {
    var centerImage = $('<img />');
    if (!parseInt(self.o.center.image.width, 10))
      self.o.center.image.width = parseInt(self.o.center.width, 10);
    ...
    Layercenter.append(
      `<div class='sw-center-empty' style='width:${parseInt(
        self.o.center.image.width,
        10
      )}%; height:${parseInt(self.o.center.image.width, 10)}%' />`
    );
  }
  if (
    typeof self.o.center.html.template === 'string' &&
    $.trim(self.o.center.html.template) !== ''
  ) {
    var centerHtml = $(`<div class='sw-center-html'>${self.o.center.html.template}</div>`);
    if (!parseInt(self.o.center.html.width, 10))
      self.o.center.html.width = parseInt(self.o.center.width, 10);
    centerHtml.css({
  ...
  $.each(self.slices, function (i, slice) {
    ...
    if (
      ...
      var LayerTitle = $('<div/>');
      if (self.toNumber(self.o.text.letterSpacing) > 0)
        textHtml.css('letter-spacing', self.toNumber(self.o.text.letterSpacing));
      LayerTitle.css({
      ...
      var LayerText = $(
        `<text stroke-width='0' data-color='${slice.color}' fill='${
          slice.color
        }' dy='${self.toNumber(
          self.o.text.offset
        )}%'><textPath xlink:href='#sw-text-${i}' startOffset='50%' style='text-anchor: middle;'>${
          slice.text
        }</textPath></text>`
      );
      LayerText.addClass('sWheel-title');
      textsGroup.css('font-size', parseInt(self.o.text.size / 2, 10));
      if (parseInt(self.o.text.letterSpacing, 10) > 0)
        textsGroup.css('letter-spacing', parseInt(self.o.text.letterSpacing / 2, 10));
  ...
  var innerLine = self.SVG('circle', {
    ...
    cy: self.half,
    r:
      parseInt(self.o.center.width, 10) +
      (parseInt(self.o.inner.width, 10) ? parseInt(self.o.inner.width, 10) / 2 - 1 : 0),
    stroke: self.o.inner.color,
    'stroke-width': parseInt(self.o.inner.width, 10),
    'fill-opacity': self.o.center.background ? 1 : 0,
    // 'fill': self.o.center.background ? self.o.center.background : '#0000'
    fill: '#0000',
  });
    ...
    },
    {
      ...
      step: function (now, fx) {
        ...
        if (self.lastStep !== self.currentStep) {
          ...
          if (
            (self.o.marker.animate === true ||
              self.o.marker.animate === 1 ||
              self.o.marker.animate === 'true') &&
            $.inArray($.trim(self.o.easing), [
              'easeInElastic',
              'easeInBack',
              'easeInBounce',
              'easeOutElastic',
            ...
            var Mduration =
              parseFloat(self.o.duration) /
              (self.totalSlices() * self.o.rotates +
                (self.totalSlices() - self.winner) -
                self.currentStep) /
              self.o.rotates;
          ...
          if ($.trim(self.o.type) === 'color') {
            ...
            self.$element
              .find('svg>g.sw-slicesGroup>path')
              .eq(self.currentSlice)
              .addClass('sw-ccurrent')
              .attr('fill', self.o.slice.selected.background);
            if (self.o.text.orientation === 'v' || self.o.text.orientation === 'vertical')
              self.$element
                .find('.sWheel-txt>.sWheel-title')
                .eq(self.currentSlice)
                .addClass('sw-ccurrent')
                .css('color', self.o.slice.selected.color);
            else
              self.$element
                .find('.sWheel-txt>.sWheel-title')
                .eq(self.currentSlice)
                .addClass('sw-ccurrent')
                .attr('fill', self.o.slice.selected.color);
          } else {
...
SuperWheel.prototype.findWinner = function (value, type) {
  ...
  $.each(self.slices, function (i, slice) {
    if (
      typeof slice[self.o.selector] === 'object' ||
      typeof slice[self.o.selector] === 'array' ||
      typeof slice[self.o.selector] === 'undefined'
    )
      return undefined;
...
SuperWheel.prototype.FontScale = function (slice) {
  var self = this;
  var Fscale =
    1 + (1 * (self.$element.width() - parseInt(self.o.width, 10))) / parseInt(self.o.width, 10);
  if (Fscale > 4) {
    Fscale = 4;
  } else if (Fscale < 0.1) {
    Fscale = 0.1;
