/**
 * Bonus system controller managing casino and sports bonuses.
 * Handles bonus creation, activation, cashback bonuses, no-deposit bonuses, bonus cancellation,
 * and bonus history tracking. Integrates with payment system for bonus distribution.
 */
"use strict";
  ...
  try {
    ...
    for (const history of validHistories) {
      ...
      if (conditions.deposit.required) {
        ...
        const totalDeposit = deposits.reduce((sum, payment) => sum + (payment.fiat_amount || payment.actually_paid || 0), 0);
...
const active = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const userCurrencySymbol = 'LU'; // Always LU
        ...
        if (matchingCurrency.deposit_amount_from > 0 || matchingCurrency.deposit_amount_to > 0) {
            // Use fiat_amount (LU) if available, otherwise actually_paid
            const depositAmount = payment.fiat_amount || payment.actually_paid || 0;
            if (depositAmount < matchingCurrency.deposit_amount_from || depositAmount > matchingCurrency.deposit_amount_to) {
        ...
        if (matchingCurrency.amount_type === 'percentage') {
            // Use fiat_amount (LU) if available, otherwise actually_paid
            const depositAmount = payment.fiat_amount || payment.actually_paid || 0;
            bonusAmount = (0, base_1.NumberFix)((depositAmount / 100) * matchingCurrency.amount, 2);
        ...
        const data = yield models_1.BonusHistories.create({
            ...
            amount: bonusAmount,
            // Use fiat_amount (LU) if available, otherwise actually_paid
            isDeposit: payment.fiat_amount || payment.actually_paid || 0,
