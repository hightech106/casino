...
const RenderCard = ({ card, style, className }: CardProps) => {
  let isHide = true;
  let Icon: any = '';
  let Color = '';
  if (card) {
    isHide = false;
    Icon = CARD_SUITS[card.suit as ISuit]?.icon;
    Color = CARD_SUITS[card.suit as ISuit]?.color;
  }
  ...
  return (
    ...
    >
      {/* Card Container */}
      <div
        className={`w-full h-full flex items-center justify-center  rounded-sm md:rounded-md  shadow-md  transition-transform delay-500 duration-300 ease-in-out ${
          isHide ? 'transform rotate-y-180' : ''
        }`}
      ...
      >
        ...
        >
          <div className="flex-col h-full w-full md:p-2 p-1" style={{ color: Color }}>
            <span className="font-bold md:text-[1.5em]">{card?.rank}</span>
